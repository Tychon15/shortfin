...
create_bind(
    vars.kbLauncher,
    hl.dsp.global("shortfin:launcher"),
    function(key)
        return normalise_keybind(key) == launcher_default and release or nil
    end
)

create_bind(vars.kbSession, hl.dsp.global("shortfin:session"))
create_bind(vars.kbClearNotifs, hl.dsp.exec_cmd("makoctl dismiss --all"), locked)
create_bind(vars.kbLock, hl.dsp.global("shortfin:lock"))

create_bind(vars.kbRestoreLock, function()
    hl.dispatch(hl.dsp.exec_cmd("qs -c shortfin -n -d"))
    hl.dispatch(hl.dsp.global("shortfin:lock"))
end)

create_bind("CTRL + SUPER + SHIFT + R", hl.dsp.exec_cmd("qs -c shortfin kill"), release)
create_bind(
    "CTRL + SUPER + ALT + R",
    hl.dsp.exec_cmd("qs -c shortfin kill; sleep .1; qs -c shortfin -n -d"),
    release
)
...
create_bind(vars.kbScreenshot, hl.dsp.exec_cmd("hyprshot -m output --clipboard-only"), locked)
create_bind(vars.kbScreenshotFreeze, hl.dsp.exec_cmd("hyprshot -m region --freeze --clipboard-only"))
create_bind(vars.kbScreenshotRegion, hl.dsp.exec_cmd("hyprshot -m region --clipboard-only"))
create_bind(vars.kbColorPicker, hl.dsp.exec_cmd("hyprpicker -a"))
create_bind(vars.kbWallpaper, hl.dsp.global("shortfin:wallpaper"))

create_bind(
    "XF86MonBrightnessUp",
    hl.dsp.exec_cmd("brightnessctl set " .. vars.brightnessStep .. "%+"),
    locked_repeating
)
create_bind(
...
create_bind(vars.kbClipboard, hl.dsp.exec_cmd("pkill fuzzel || shortfin-clipboard"))
create_bind(vars.kbClipboardDel, hl.dsp.exec_cmd("pkill fuzzel || shortfin-clipboard --delete"))
create_bind(vars.kbEmoji, hl.dsp.exec_cmd("pkill fuzzel || shortfin-emoji"))
create_bind(
